...
export const PageNotFound = ({
	title = "Checkout nicht gefunden",
	message = "Deine Checkout-Sitzung wurde nicht gefunden. Sie ist möglicherweise abgelaufen oder wurde bereits abgeschlossen.",
}: PageNotFoundProps) => {
	return (
		<div className="min-h-screen bg-secondary">
			...
			<header className="border-b border-border bg-background">
				<div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
					<Link href={`/${process.env.NEXT_PUBLIC_DEFAULT_CHANNEL ?? "default-channel"}`} className="flex items-center">
				...
				<div className="mx-auto max-w-md">
					<div className="rounded-lg border border-border bg-card p-8 text-center shadow-sm">
						...
						<div className="flex flex-col gap-3">
							<Link
								href={`/${process.env.NEXT_PUBLIC_DEFAULT_CHANNEL ?? "default-channel"}`}
					...
					<p className="mt-6 text-center text-sm text-muted-foreground">
						Falls du glaubst, dass dies ein Fehler ist, versuche bitte erneut, Artikel in deinen Warenkorb zu legen, oder kontaktiere den Support.
					</p>
				</div>
			</main>
		</div>
